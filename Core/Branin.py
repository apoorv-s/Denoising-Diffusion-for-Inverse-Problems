...
class DiffusionModel(nn.Module):
    ...
    def forward(self, inp, out):
        ...
        sqrt_alpha_bar, sqrt_om_alpha_bar=diffusion_noise_schedule(self.config.n_time,
                                                                   self.config.beta_range)
        inp_noise=torch.randn_like(inp)
        t_indices=torch.randint(0, self.config.n_time, (batch_size, 1))
        noisy_x = inp*sqrt_alpha_bar[t_indices] + inp_noise*sqrt_om_alpha_bar[t_indices]
        
        predicted_noise=self.nn_model(noisy_x, out, t_indices/self.config.n_time)
    ...
    def train(self):
        ...
        for ep in trange(self.config.n_epochs):
            ...
            for ind, batch_data in enumerate(dataloader):
                optimizer.zero_grad()
                loss=self.model(batch_data['x'], batch_data['y'])
                loss.backward()
                optimizer.step()
